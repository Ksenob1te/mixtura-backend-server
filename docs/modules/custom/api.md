...
### Behavior
- Проверяет, что оценка находится в пределах `[min_rating, max_rating]` рейтинг-сета сервера.
...
| Exception | Condition |
|-----------|-----------|
| `NotFoundException` | Кастомный список не найден; `game_role_id` не найден в БД |
| `ForbiddenException` | Участник не является создателем списка и не имеет права `EDIT_ALL_CUSTOMS` |
| `BadRequestException` | Значение `rating` выходит за пределы `[min_rating, max_rating]`, заданные рейтинг-сетом сервера |
| `InternalLogicException` | Ошибка создания записи рейтинга в БД (нарушение уникальности или прочее); сервер не найден для участника |
