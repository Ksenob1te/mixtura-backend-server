...
from ...dependency import CoreServiceDependency, GameRoleServiceDependency
...
async def get_role_set(
    ...
    role_set = await game_role_service.get_role_set_for_server(
        data.access_data.server_id
    )
    return ResponseMessage(
        status=200, message=GameRoleSetResponse.model_validate(role_set)
    )
...
async def create_role(
    ...
    return ResponseMessage(
        status=200, message=GameRoleItemResponse.model_validate(role)
    )
