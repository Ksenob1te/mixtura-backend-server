...
from ...dependency import CoreServiceDependency, RatingServiceDependency
...
async def get_rating_set(
    ...
    return ResponseMessage(
        status=200, message=RatingSetResponse.model_validate(rating_set)
    )
...
async def create_rating(
    ...
    return ResponseMessage(
        status=200, message=RatingItemResponse.model_validate(rating)
    )
